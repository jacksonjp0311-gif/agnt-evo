...
[![AGNT Plugin](https://img.shields.io/badge/AGNT-Plugin-blue.svg)](https://agnt.gg)
[![Version](https://img.shields.io/badge/version-3.0.0-green.svg)](https://github.com/jacksonjp0311-gif/CHEMIFRAME-AGENT-PLUGIN)
[![License](https://img.shields.io/badge/license-MIT-yellow.svg)](LICENSE)
[![Python](https://img.shields.io/badge/python-3.8%2B-brightgreen.svg)](https://python.org)

**CHEMIFRAME** is a self-contained AGNT plugin that gives AI agents the ability to program chemistry at a high level. Describe what molecule you want to synthesize, what starting materials you have, and what constraints matter — and get back a fully verified synthesis route with safety contracts, simulation results, and executable XDL protocols.
...
|------|---------|:---:|
| **CHEMIFRAME Demo** | Run pre-built demos (Suzuki coupling, oligonucleotide synthesis, chemo-bio hybrid) | ✅ |
| **CHEMIFRAME Blueprints** | List all 14 reaction blueprints with yields, catalysts, temperatures | ❌ |
| **CHEMIFRAME Compile** | Full pipeline: intent → route → contracts → XDL | ✅ |
| **CHEMIFRAME Validate** | Contract verification only (4 safety checks) | ✅ |
...
Three-tier simulation backend — automatically falls back if a tier is unavailable:

| Tier | Engine | Requirements | Capabilities |
|------|--------|-------------|--------------|
| **Tier 1** | RDKit (local) | `pip install rdkit-pypi` | Reaction SMARTS validation, product prediction, molecular properties |
| **Tier 2** | IBM RXN API (cloud) | `RXN4CHEMISTRY_API_KEY` env var | AI-powered reaction prediction and retrosynthesis |
| **Tier 3** | Rule-based (always available) | None | Deterministic simulation with chemically meaningful DEC events |

**DEC events** (Decision Event Chain) are real-time monitoring actions — e.g., "if conversion is below threshold, extend heating window by 30 minutes." These are the building blocks of adaptive lab automation.

---

## Contract-First Safety

Every route passes **4 verification contracts** before execution. This is what makes CHEMIFRAME safe for AI agents to use:

| Contract | What It Checks |
|----------|---------------|
| **Detectability** | Can we analytically monitor this reaction? (e.g., HPLC, NMR, IR) |
| **Assembly Bound** | Are the assembly steps bounded and finite? |
| **Decision Points** | Are observation/monitor points defined at each step? |
| **Route Admissibility** | Is the route structurally valid for the target domain? |

If any contract fails, the route is rejected with a detailed explanation — no unsafe protocols ever reach execution.
...
│   │   ├── base.py
│   │   ├── coupling.py
│   │   ├── expanded.py        ← Grignard, Diels-Alder, Click, etc.
...
│   │   ├── ir.py
│   │   ├── lower_to_xdl.py
│   │   ├── lower_to_hybrid_ir.py
│   │   ├── lower_to_sequence_ir.py
│   │   ├── chempile_bind.py
│   │   └── artifact_store.py
...
│   │   ├── assembly_bound.py
│   │   ├── dec_points.py
│   │   ├── route_admissibility.py
│   │   ├── sequence_admissibility.py
│   │   ├── hybrid_admissibility.py
│   │   └── transfer_admissibility.py
...
│       ├── chemistry_simulator.py  ← 3-tier simulation engine
│       ├── simulator.py
│       ├── domain_adapters.py
...
| **RDKit** | Optional | Tier 1 local simulation (`pip install rdkit-pypi`) |
| **IBM RXN API Key** | Optional | Tier 2 cloud simulation (set `RXN4CHEMISTRY_API_KEY`) |
