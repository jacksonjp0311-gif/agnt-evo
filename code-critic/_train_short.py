"""Short training run — 10 epochs, 2000 samples. Should complete in ~3-5 min."""
import sys, os, time
sys.stdout = open('_train_short_log.txt', 'w', buffering=1)
...
print("SHORT TRAINING — 10 epochs, 2000 samples")
print("="*60)

# Generate data
feat, qual, issues = generate_dataset(n_samples=2000, augment=True)
...
sch = optim.lr_scheduler.CosineAnnealingLR(opt, T_max=E)
...
for ep in range(E):
    ...
    print(f"  Epoch {ep+1:2d}/{E}  train={atl:.4f}  val={vl:.4f}  mae={vmae:.4f}  lr={sch.get_last_lr()[0]:.2e}")
