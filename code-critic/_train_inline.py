"""Inline training — runs the entire training loop in one script."""
...
sch = optim.lr_scheduler.OneCycleLR(opt, max_lr=LR, epochs=E, steps_per_epoch=len(TL), pct_start=W/E)
ql = nn.HuberLoss(delta=0.12)
il = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([2.5, 1.5, 1.5, 3.0, 1.2, 1.3]))

print(f"Training {E} epochs, batch 64...")
...
for ep in range(E):
    ...
    if (ep+1)%10==0 or ep<3:
        ...
        print(f"  Epoch {ep+1:3d}/{E}  train={atl:.4f}  val={vl:.4f}  mae={vmae:.4f}  lr={lr:.2e}")
...
# Quick test
print("\nTEST")
m = CodeCriticV3.__new__(CodeCriticV3)
m.__init__()
...
for name, code in tests:
    ...
    print(f"{name}: q={out['quality_score'].item():.3f} probs={torch.softmax(out['issue_logits'],-1).squeeze(0).numpy().round(2)}")
